...
class CPUMonitor:
    def __init__(self, node=None, interval=0.02):
        self.node = node
    ...
    def _background_log(self):
        while True:
            try:
                ...
                top3 = sorted(threads, key=lambda t: t.user_time + t.system_time, reverse=True)[:3]
                top3_str = ", ".join([f"tid={t.id}(usr={t.user_time:.1f}s,sys={t.system_time:.1f}s)" for t in top3])
                extra_info = f"threads: {thread_count} | fds: {num_fds} | top3_threads: {top3_str}"
                if self.node is not None:
                    costmap_count = self.node._costmap_cb_count
                    costmap_time = self.node._costmap_cb_time_total
                    tf_count = self.node._tf_cb_count
                    tf_time = self.node._tf_cb_time_total
                    self.node._costmap_cb_count = 0
                    self.node._costmap_cb_time_total = 0.0
                    self.node._tf_cb_count = 0
                    self.node._tf_cb_time_total = 0.0
                    print(f"[CPU Monitor] cpu: {cpu:.1f}%, mem: {mem:.1f} MB | costmap_cb: {costmap_count} calls, {costmap_time:.3f}s total | tf_cb: {tf_count} calls, {tf_time:.3f}s total | {extra_info}")
                else:
                    print(f"[CPU Monitor] cpu: {cpu:.1f}%, mem: {mem:.1f} MB | {extra_info}")
...
class CarAvoidancePointActionServer(Node):
    def __init__(self):
        ...
        self._tf_cb_count = 0
        self._costmap_cb_time_total = 0.0
        self._tf_cb_time_total = 0.0
        ...
        self.cpu_monitor.start_background_logging()
    ...
    def global_costmap_callback(self, msg):
        ...
        if self.show_global_costmap_raw_colored_cv2 or self.show_global_costmap_raw_cv2:
            cv2.waitKey(1)
        self._costmap_cb_time_total += time.monotonic() - start
    ...
    def get_robot_pose_timer_callback(self):
        ...
        try:
            ...
            self._tf_cb_time_total += time.monotonic() - start
